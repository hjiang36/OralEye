...
class WiFiCharacteristic(dbus.service.Object):
    ...
    def get_wifi_status(self):
        try:
            ...
            print(f"ssid: {ssid}, IP: {ip_address}")
    ...
    def get_properties(self):
        ...
        return {
            ...
            'Value': dbus.Array([dbus.Byte(c) for c in value.encode()], signature='y'),
            'Flags': self.flags,
        }
    ...
    def ReadValue(self, options):
        try:
            value = self.get_wifi_status()
            return [dbus.Byte(c) for c in value.encode('utf-8')]
        except Exception as e:
            print(f"Error in ReadValue: {e}")
            raise dbus.exceptions.DBusException("Error reading value", name="org.freedesktop.DBus.Error.Failed")
    ...
    def get_properties(self):
        return {
            ...
            'Characteristics': dbus.Array(
                [dbus.ObjectPath(char.path) for char in self.characteristics],
                signature='o'
            ),
            'Type': self.type
        }
...
class Application(dbus.service.Object):
    ...
    def GetManagedObjects(self):
        ...
        for service in self.services:
            ...
            response[dbus.ObjectPath(service_path)] = {
                'org.bluez.GattService1': service.get_properties()
            }
            for characteristic in service.characteristics:
                char_path = characteristic.path
                response[dbus.ObjectPath(char_path)] = {
                    'org.bluez.GattCharacteristic1': characteristic.get_properties()
                }
        return response
